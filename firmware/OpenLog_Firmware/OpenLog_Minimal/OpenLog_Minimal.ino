...
SerialPort<0, 1024, 15> NewSerial;  //dch
...
#endif
  NewSerial.print("AT+VOL=25\r\n"); //max 30
  delay(100);
  NewSerial.println("AT+PLAYMODE=3\r\n"); //set to play one track then pause
  delay(100);
}
...
byte appendFile(char* fileName) {
  ...
  while (1) {
    ...
    if (charsToRecord > 0) {
      ...
      for (int i; i < charsToRecord; i++) {
        if (buff[i] = '~') {
          soundFound++;
        }else if (soundFound) {
          if (buff[i] >= '0' and buff[i] <= '9') {
            sound = (sound * 10) + (buff[i] - '0');
          }
          else{
            soundFound=0;
            if (sound>0){
              NewSerial.print(F("AT+PLAYNUM="));
              NewSerial.println(sound);
              sound=0;
            }
          }
        }
